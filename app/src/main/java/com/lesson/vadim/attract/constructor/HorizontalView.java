package com.lesson.vadim.attract.constructor;
...
public class HorizontalView extends AdapterView {
    ...
    protected int mCurrentX; protected int mNextX;
    ...
    private DataSetObserver mDataObserver = new DataSetObserver() {
        ...
        @Override
        public void onChanged() { synchronized (HorizontalView.this)
        {
            mDataChanged = true;
        }
            invalidate();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mAdapter == null) {
            ...
            int oldCurrentX = mCurrentX; initView();
            removeAllViewsInLayout();
            mNextX = oldCurrentX;
            mDataChanged = false;
    ...
    private void fillList(final int dx) {
        int edge = 0; View child = getChildAt(getChildCount() - 1);
        if (child != null) {
            edge = child.getRight();
        }
        fillListRight(edge, dx);
        edge = 0; child = getChildAt(0);
        if (child != null) {
            edge = child.getLeft();
        }
    ...
    public synchronized void scrollTo(int x) {
        mScroller.startScroll(mNextX, 0, x - mNextX, 0); requestLayout();
    }
    ...
    private GestureDetector.OnGestureListener mOnGesture = new GestureDetector.SimpleOnGestureListener() {
        ...
        { synchronized (HorizontalView.this) { mNextX += (int) distanceX; } requestLayout(); return true; }

        @Override
        public boolean onSingleTapConfirmed(MotionEvent e)
        { for (int i = 0; i < getChildCount(); i++)

        { View child = getChildAt(i); if (isEventWithinView(e, child))
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (isEventWithinView(e, child)) {
                    if (mOnItemLongClicked != null) {
                        mOnItemLongClicked.onItemLongClick(HorizontalView.this, child,
